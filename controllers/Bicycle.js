...
import PackagesBicycleModel from "../models/packageBicycle.js";
import packageBicycle from "../models/packageBicycle.js";
...
export const create = async (req, res) => {
    try {
        ...
        if (oldBicycle) {
            return res.status(400).json({message: 'Велосипед с таким именим уже есть'})
        }
        ...
        const bicycle = await doc.save()

        await packages.map(async (pack) => {
            const packDoc = PackagesBicycleModel({
                packageId: pack, bicycleId: bicycle._id.toString(),
            })
            await packDoc.save()
        })
        res.json(bicycle)
...
export const update = async (req, res) => {
    try {
        ...
        if(oldBicycle && oldBicycle._id.toString() !== bicycleId){
                return res.status(400).json({message: "Продукт с таким именим уже существует"})
        }
        ...
        await packages.map(async (pack) => {
            const packDoc = PackagesBicycleModel({
                packageId: pack, bicycleId: bicycleId,
            })
            await packDoc.save()
        })
...
export const get = async (req, res) => {
    try{
        ...
        if(!bicycle) return res.status(404).json({message: 'Велосипед не найден'})
...
export const getByPackage = async (req, res) => {
    try{
        ...
        for(const bicyclePackage of bicyclePackageList){
            const bicycle = await BicycleModel.findOne({_id: bicyclePackage.bicycleId.toString()})
